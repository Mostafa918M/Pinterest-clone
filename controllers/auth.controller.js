...
async function createAndSendEmailCode(user, logger) {
  ...
  logger.info("Auth: verification code sent", {
...
const signup = asyncErrorHandler(async (req, res, next) => {
  ...
  const { valid, errors } = passwordValidator.validatePasswordUsingLib(
    password,
    { username, email }
  );
  if (!valid) {
  ...
  sendResponse(
    ...
    201,
    "success",
    "Signup successful. Please verify your email with the code sent.",
    {
      user: {
        id: user._id,
...
const signin = asyncErrorHandler(async (req, res, next) => {
  ...
  if (!email || !password) {
    logger.warn("Auth: signin missing fields", {
      email,
      hasEmail: !!email,
  ...
  if (!user) {
    logger.warn("Auth: signin invalid email", {
      email,
  ...
  if (!user.password) {
    return next(
      new ApiError(
        "This account uses Google Sign-In. Use 'Continue with Google' or set a password.",
        401
      )
    );
  }
  ...
  if (!user.isEmailVerified) {
    ...
    if (
      ...
      (now - new Date(user.emailVerificationLastSentAt)) / 1000 <
      ...
      return sendResponse(
        ...
        202,
        "pending",
        "Email not verified. Check your inbox for the latest code.",
        {
...
const callback = asyncErrorHandler(async (req, res, next) => {
  ...
  if (!email || !googleId || !email_verified)
    return next(new ApiError("Unable to authenticate with Google"));
...
const verifyEmail = asyncErrorHandler(async (req, res, next) => {
  ...
  if (
    ...
    return next(
      new ApiError("Verification code expired. Please request a new code.")
    );
  }

  if (user.emailVerificationAttempts >= MAX_VERIFICATION_ATTEMPTS) {
    return next(
      new ApiError("Too many failed attempts. Please request a new code.")
    );
  }
  ...
  if (!ok) {
    await user.save();
    logger.warn("Auth: verify email failed attempt", {
      email,
  ...
  try {
    ...
    if (authHeader) {
      ...
      try {
        ...
        logger.info("Auth: tokenVersion incremented on signout", {
          userId: decoded.id,
        });
      } catch (err) {
        logger.warn("Auth: signout token verification failed", {
          message: err.message,
          stack: err.stack,
        });
        return next(
...
const forgetPassword = asyncErrorHandler(async (req, res, next) => {
  ...
  if (
    ...
    (now - new Date(user.passwordResetLastSentAt)) / 1000 <
  ...
  user.passwordResetExpiresAt = new Date(
    now.getTime() + RESET_TOKEN_TTL_MIN * 60 * 1000
  );
...
const resetPassword = asyncErrorHandler(async (req, res, next) => {
  ...
  const { valid, errors } =
    passwordValidator.validatePasswordUsingLib(newPassword);
  if (!valid) {
  ...
  logger.info("Auth: password reset success", {
    userId: user._id,
    email: user.email,
  });
...
const newAccessToken = asyncErrorHandler(async (req, res, next) => {
  ...
  logger.info("Auth: new access token generated", {
    userId: user._id.toString(),
  });
